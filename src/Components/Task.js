  ...
  return (
    <div>
      {toDoArray.map((elemnt) => (
        <div className="task-div" key={elemnt.identification}>
          ...
          <div className="scores">
            ...
            {/* } <button>Edit</button> {*/}
            <button onClick={(e) => handleDeleteTask(elemnt.id,e,elemnt.identification ) } className='delete-button' style={{cursor: 'pointer'}}>Delete</button>
          </div>
        </div>
      ))}
    </div>
